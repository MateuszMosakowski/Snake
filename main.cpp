...
enum class Typ_pola { puste, sciana, waz, owoc,};
...
public:
    ...
    void graj() {
        ...
        bool wygrana = false;
        wynik = 0;
        int max_rozmiar = (plansza.getSzerokosc() - 2) * (plansza.getWysokosc() - 2);
        ...
        while (!koniec) {
            ...
            if (waz.pobierz_kierunek() != Ruchy::brak) {
                ...
                if (zjadl) {
                    wynik += 10;
                    if (waz.pobierz_cialo().size() == max_rozmiar) {
                        wygrana = true;
                        koniec = true;
                    }
                    else {
        ...
        if (wygrana) {
            cout << "GRATULACJE! Wygrales! Wynik: " << wynik << endl;
        }
        else {
